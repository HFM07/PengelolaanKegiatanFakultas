    ...
    <div class="container">
        ...
        <form action="{{ route('kegiatan.tambahKegiatan') }}" method="POST">
            ...
            <div class="mb-3 row">
                ...
                <select name="NamaKegiatan" id="NamaKegiatan" class="Textbox">
                    @foreach ($kegiatan as $item => $i)
                        <option value="{{ $i->NamaKegiatan }}">{{ $i->Id }} : {{ $i->NamaKegiatan }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3 row">
                <label for="BidangKegiatan" class="form-label text-dark">Bidang Kegiatan</label>
                <select name="BidangKegiatan" id="BidangKegiatan" class="Textbox">
                    @foreach ($kegiatan as $item => $i)
                        <option value="{{ $i->BidangKegiatan }}">{{ $i->Id }} : {{ $i->BidangKegiatan }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3 row">
                <label for="TglMulai" class="form-label text-dark">Tanggal Mulai</label>
                <select name="TglMulai" id="TglMulai" class="Textbox">
                    @foreach ($kegiatan as $item => $i)
                        <option value="{{ $i->TglMulai }}">{{ $i->Id }} : {{ $i->TglMulai }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3 row">
                <label for="TglSelesai" class="form-label text-dark">Tanggal Selesai</label>
                <select name="TglSelesai" id="TglSelesai" class="Textbox">
                    @foreach ($kegiatan as $item => $i)
                        <option value="{{ $i->TglSelesai }}">{{ $i->Id }} : {{ $i->TglSelesai }}</option>
                    @endforeach
                </select>
            </div>
            <div class="mb-3 row">
